...
malicious_counts_a = malicious_counts_b = malicious_counts_c = malicious_counts_d = 0
...
with open(PAPER_PATH, 'r') as f:
    ...
    for row in reader:
        ...
        if paper_verdict == '0' and curr_verdict == '0':
            both_nonremoved += 1
            malicious_counts_a += malicious_cnt
        elif paper_verdict == '1' and curr_verdict == '1':
            both_removed += 1
            malicious_counts_b += malicious_cnt
        elif paper_verdict == '0' and curr_verdict == '1':
            nonremoved2removed += 1
            malicious_counts_c += malicious_cnt
        else:
            removed2nonremoved += 1
            malicious_counts_d += malicious_cnt
...
print(f'Not removed by both paper and our approach: {malicious_counts_a} ({malicious_counts_a/both_nonremoved})')
print(f'Removed by both paper and our approach: {malicious_counts_b} ({malicious_counts_b/both_removed})')
print(f'Not removed by paper but removed by our approach: {malicious_counts_c} ({malicious_counts_c/nonremoved2removed})')
print(f'Removed by paper but not removed by our approach: {malicious_counts_d} ({malicious_counts_d/removed2nonremoved})')